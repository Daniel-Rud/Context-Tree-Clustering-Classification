...
In this document, we provide code for the simulations described in the manuscript \textit{Context Tree Clustering and Classification}. We first introduce the context tree structural dissimilarity measures.  
...
First, we define two generating VLMCs from two seperate state sequences. vlmcA and vlmcB serve as the two \textit{population} context trees from which we will simulate state sequences from.  
...
Here, we compute the distance $D_{\alpha_{|\chi|}}^*$.  Note that the for loop over alpha divisor allows the user to investigate a set of alpha divisor values.  However, only the alpha divisor $|\chi|^2/4$ is used.  
